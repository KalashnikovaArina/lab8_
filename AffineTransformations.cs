...
internal class AffineTransformations
{
    ...
    public static List<XYZPoint> RecalculatedCoords(Polyhedron polyhedron, double[,] matrix)
    {
        List<XYZPoint> newVertices = new List<XYZPoint>(polyhedron.points);
        for (int i = 0; i < polyhedron.points.Count; i++)
        {
            double[,] oldCoords = new double[,] { { polyhedron.points[i].x, polyhedron.points[i].y, polyhedron.points[i].z, 1 } };
            double[,] newCoords = Multiply(oldCoords, matrix);
            newVertices[i] = new XYZPoint(newCoords[0, 0] / newCoords[0, 3], newCoords[0, 1] / newCoords[0, 3], newCoords[0, 2] / newCoords[0, 3]);
    ...
    /// <param name="polyhedron">Многогранник</param>
    /// <param name="matrix">Матрица аффинного преобразования</param>
    private static void RecalculateCoords(ref Polyhedron polyhedron, double[,] matrix) => polyhedron.points = RecalculatedCoords(polyhedron, matrix);
    ...
    public static double[,] CalculateCenterCoords(Polyhedron polyhedron)
    {
        ...
        foreach (XYZPoint vertex in polyhedron.points)
        {
