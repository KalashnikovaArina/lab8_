    ...
    internal class ZBuffer
    {
        ...
        public static Bitmap ZBuff(double[,] matrix, ListBox.ObjectCollection polyhedrons, int width, int height)
        {
            ...
            for (var ind = 0; ind < polyhedrons.Count; ind++)
            {
                ...
                var orig_verts = (polyhedrons[ind] as Polyhedron).points;
                List<XYZPoint> new_verts = new List<XYZPoint> { };
                foreach (var vert in orig_verts)
                {
                    ...
                    new_verts.Add(new XYZPoint(cur_m[0, 0], cur_m[0, 1], vert.z));
                }
                
                // Разобъём грани на треугольники для более удобной растеризации
                var triangulated_faces = new List<List<XYZPoint>>();
                foreach (var face in (polyhedrons[ind] as Polyhedron).verges)
                {
        ...
        private static List<List<XYZPoint>> Triangulate(List<XYZPoint> all_verts, List<int> picked_verts)
        {
            List<List<XYZPoint>> triangulations = new List<List<XYZPoint>> { };
            for (int i = 2; i < picked_verts.Count; i++)
            {
                triangulations.Add(new List<XYZPoint> { all_verts[picked_verts[0]], all_verts[picked_verts[i - 1]], all_verts[picked_verts[i]] });
            }
            return triangulations;
        }

        private static List<XYZPoint> Rasterization(List<XYZPoint> triag)
        {
            
            triag.OrderBy(v => v.y);
            var up = triag[0]; var mid = triag[1]; var bot = triag[2];
            List<XYZPoint> all_triag_pix = new List<XYZPoint>();
