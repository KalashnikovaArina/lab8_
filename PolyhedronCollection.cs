...
namespace LAB7 {
    ...
    internal static class PolyhedronCollection
    {
        ...
        {
            XYZPoint start = new XYZPoint(-75, -75, -75);  //=(250, 150, 200)?
            double side_size = 150;

            List<XYZPoint> vertices = new List<XYZPoint>() {
                start,
                new XYZPoint(start.x + side_size, start.y + 0,         start.z + side_size),
                new XYZPoint(start.x + side_size, start.y + side_size, start.z + 0),
                new XYZPoint(start.x + 0,         start.y + side_size, start.z + side_size),
            };
        ...
        {
            XYZPoint start = new XYZPoint(300, 300, 0);  
            double side_size = 150;

            List<XYZPoint> vertices = new List<XYZPoint>() {
                start,
                new XYZPoint(start.x + side_size, start.y + 0,         start.z + 0),
                new XYZPoint(start.x + 0,         start.y + side_size, start.z + 0),
                new XYZPoint(start.x + 0,         start.y + 0,         start.z + side_size),

                new XYZPoint(start.x + side_size, start.y + side_size, start.z + 0),
                new XYZPoint(start.x + 0,         start.y + side_size, start.z + side_size),
                new XYZPoint(start.x + side_size, start.y + 0,         start.z + side_size),

                new XYZPoint(start.x + side_size, start.y + side_size, start.z + side_size)
            };
        ...
        {            
            ...
            List<XYZPoint> vertices = new List<XYZPoint>() {
                new XYZPoint(start.x + 0,         start.y - side_size, start.z - r),
                new XYZPoint(start.x + 0,         start.y + side_size, start.z - r),
                new XYZPoint(start.x + side_size, start.y + r,         start.z + 0),
                new XYZPoint(start.x + r,         start.y + 0,         start.z - side_size),
                new XYZPoint(start.x + side_size, start.y - r,         start.z + 0),
                new XYZPoint(start.x - side_size, start.y - r,         start.z + 0),
                new XYZPoint(start.x - r,         start.y + 0,         start.z - side_size),
                new XYZPoint(start.x - side_size, start.y + r,         start.z + 0),
        ...
        public static Polyhedron MakeDodecahedron() {
            ...
            List<XYZPoint> vertices = new List<XYZPoint>() {
                new XYZPoint(start.x + 0,         start.y - side_size, start.z - r),
                new XYZPoint(start.x + 0,         start.y + side_size, start.z - r),
                new XYZPoint(start.x + x,         start.y + x,         start.z - x),
                new XYZPoint(start.x + r,         start.y + 0,         start.z - side_size),
                new XYZPoint(start.x + x,         start.y - x,         start.z - x),
                new XYZPoint(start.x + side_size, start.y - r,         start.z + 0),
                new XYZPoint(start.x - side_size, start.y - r,         start.z + 0),
        ...
        public static Polyhedron MakeRotationFigure(string axis, int partitions_count, List<XYZPoint> forming_verts)
        {
            int forming_count = forming_verts.Count;
            double angle = 360.0 / partitions_count;
            
            var all_verts = new List<XYZPoint>(forming_verts);
            ...
            for (int i = 0; i < partitions_count - 1; i++)
            {
                ...
                foreach (var vert in temp_poly.points)
                {
                    all_verts.Add(new XYZPoint(vert));
                }
            }
